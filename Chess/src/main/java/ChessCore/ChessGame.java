...
public abstract class ChessGame {
   ...
   public ChessBoard getBoard() {
    ...
    public boolean makeMove(Move move) {
        ...
        caretaker.saveState(new ChessMemento(getBoard()));
         
          
        
         
          System.out.println(caretaker.history.peek().getBoard().getPieceAtSquare(fromSquare) + "   before");
        board.setPieceAtSquare(fromSquare, null);
        board.setPieceAtSquare(move.getToSquare(), fromPiece);
         
                      caretaker.saveState(new ChessMemento(getBoard()));

            System.out.println(caretaker.history.peek().getBoard().getPieceAtSquare(fromSquare)+" after move");
   ...
   public void printBoard() {
        ...
        for (int row = 7; row >= 0; row--) {
            for (int col = 0; col < 8; col++) {
                ...
                if (piece != null) {
                    System.out.print(piece.getOwner() + " " + piece.getClass().getSimpleName() + "\t");
                } else {
